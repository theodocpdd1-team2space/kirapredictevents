  ...
  <div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between px-2">
    ...
    <div class="flex items-center gap-3 w-full md:w-auto">
      <a href="{{ route('estimations.index') }}"
         class="flex-1 md:flex-none inline-flex items-center justify-center gap-2 rounded-2xl bg-white dark:bg-slate-800 px-5 py-3 text-sm font-semibold text-slate-700 dark:text-slate-200 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] hover:shadow-md transition-all duration-300">
      ...
      <a href="{{ route('events.create') }}"
         class="flex-1 md:flex-none inline-flex items-center justify-center gap-2 rounded-2xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 hover:bg-blue-700 hover:-translate-y-0.5 transition-all duration-300">
        ...
        <div>
          ...
          <div class="mt-3 inline-flex items-center gap-1.5 rounded-full {{ $monthlyGrowthPercent >= 0 ? 'bg-white/20' : 'bg-rose-500/30' }} px-2.5 py-1 text-xs font-medium backdrop-blur-sm">
            @if($monthlyGrowthPercent >= 0)
              <!-- Panah Naik -->
              <svg class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M12 7a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0V8.414l-4.293 4.293a1 1 0 01-1.414 0L8 10.414l-4.293 4.293a1 1 0 01-1.414-1.414l5-5a1 1 0 011.414 0L11 10.586 14.586 7H12z" clip-rule="evenodd"/></svg>
              +{{ $monthlyGrowthPercent }}% bulan ini
            @else
              <!-- Panah Turun -->
              <svg class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8 13a1 1 0 110 2H3a1 1 0 01-1-1v-5a1 1 0 112 0v4.586l4.293-4.293a1 1 0 011.414 0L12 9.586l4.293-4.293a1 1 0 011.414 1.414l-5 5a1 1 0 01-1.414 0L9 9.414 5.414 13H8z" clip-rule="evenodd"/></svg>
              {{ $monthlyGrowthPercent }}% bulan ini
            @endif
          </div>
        </div>
    ...
    <div class="rounded-[24px] bg-white dark:bg-slate-900 p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:-translate-y-1 transition-transform duration-300">
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="mt-3 text-xs text-slate-500"><span class="font-semibold text-emerald-500">{{ number_format($approvedEstimations) }}</span> estimasi goal</p>
        </div>
    ...
    <div class="rounded-[24px] bg-white dark:bg-slate-900 p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:-translate-y-1 transition-transform duration-300">
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="mt-3 text-xs text-slate-500"><span class="font-semibold text-amber-500">{{ number_format($pendingEstimations) }}</span> butuh follow up</p>
        </div>
    ...
    <div class="rounded-[24px] bg-white dark:bg-slate-900 p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:-translate-y-1 transition-transform duration-300">
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="mt-3 text-xs text-slate-500"><span class="font-semibold text-blue-500">{{ number_format($monthlyEstimations) }}</span> estimasi baru</p>
        </div>
  ...
  <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
    ...
    <div class="xl:col-span-2 rounded-[24px] bg-white dark:bg-slate-900 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
      <div class="flex items-center justify-between p-6">
        ...
        <a href="{{ route('estimations.index') }}" class="inline-flex h-9 items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-800 px-4 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 transition-colors">
          View All
        </a>
      </div>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody>
            @forelse($recentEstimations as $e)
              ...
              @endphp
              <tr onclick="window.location='{{ route('estimations.show', $e->id) }}'" class="group cursor-pointer hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                <td class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/60">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      <div class="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">{{ $eventLabel }}</div>
                      <div class="text-xs text-slate-500 mt-0.5">{{ $eventMeta ?: 'Detail belum lengkap' }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/60 whitespace-nowrap">
                  <div class="text-sm text-slate-700 dark:text-slate-300 font-medium">{{ optional($e->created_at)->format('d M Y') }}</div>
                  <div class="text-xs text-slate-400">{{ optional($e->created_at)->format('H:i') }} WIB</div>
                </td>
                <td class="px-6 py-4 border-b border-slate-50 dark:border-slate-800/60 whitespace-nowrap">
    ...
    <div class="space-y-8">
      ...
      <div class="rounded-[24px] bg-white dark:bg-slate-900 p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
        ...
        <div class="space-y-3">
          <a href="{{ route('estimations.index', ['status' => 'pending']) }}" class="group flex items-center justify-between rounded-2xl p-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <div class="flex items-center gap-3">
              <div class="rounded-xl bg-amber-50 p-2.5 text-amber-500">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
              </div>
              <div>
          ...
          <a href="{{ route('estimations.index') }}" class="group flex items-center justify-between rounded-2xl p-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            <div class="flex items-center gap-3">
              <div class="rounded-xl bg-rose-50 p-2.5 text-rose-500">
                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
              </div>
              <div>
      ...
      <div class="rounded-[24px] bg-white dark:bg-slate-900 p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
        <div class="flex items-center justify-between mb-5">
          ...
          <a href="{{ route('inventories.index') }}" class="text-xs font-semibold text-blue-600 hover:underline">Kelola</a>
        </div>
        ...
        <div class="space-y-5">
          @forelse($topEquipments as $index => $item)
            ...
            <div>
              <div class="flex items-center justify-between text-sm mb-2">
                <div class="font-medium text-slate-700 dark:text-slate-200 truncate pr-4">{{ $item->equipment_name }}</div>
                <div class="text-slate-400 font-semibold text-xs">{{ number_format((int)$item->used_count) }}x</div>
              </div>
          ...
          @empty
            <div class="text-center text-sm text-slate-500 py-4">Belum ada data pemakaian.</div>
          @endforelse
        </div>
      </div>
